import { api, authToken, expect, loginAndGetTokens } from 'test/setup'

const randomEmail = (prefix) =>
  `${prefix}_${Date.now()}_${Math.random().toString(16).slice(2)}@example.com`

const wait = (ms) => new Promise((resolve) => setTimeout(resolve, ms))

const waitForVerificationToken = async ({ email, status = 'unverified', type, userId }) => {
  ...
  if (!token) {
    const message =
      lastError?.response?.data?.message || `VERIFICATION_TOKEN_NOT_FOUND_FOR_${type?.toUpperCase?.()}`
...
const registerTestUser = async ({ prefix, password, verify = false }) => {
  ...
  return { email, password, userId: verificationToken.user_id, verificationToken: verificationToken.token }
}
...
describe('User Mutation Tests', () => {
  ...
  before(async () => {
    ...
    context.adminManaged = await registerTestUser({ prefix: 'admin-managed', password: 'Admin123!@#', verify: true })
  ...
  describe('POST /users/register', () => {
    it('registers a user successfully', async () => {
      ...
      expect(response.data.data).to.include({ email })

      const verificationToken = await waitForVerificationToken({ email, type: 'user_verification' })
      context.register = { ...payload, userId: verificationToken.user_id, verificationToken: verificationToken.token }
    })
  ...
  describe('POST /users/verify-user-email', () => {
    it('verifies user email successfully', async () => {
      const response = await api.post('/users/verify-user-email', {
        email: context.register.email,
        token: context.register.verificationToken
      })

      expect(response.status).to.equal(200)
      expect(response.data.data).to.include({ email: context.register.email, status: 'active' })
    })
  ...
  describe('POST /users/resend-verification-email', () => {
    ...
    it('returns error when user is already verified', async () => {
      ...
      try {
        await api.post('/users/resend-verification-email', { email: context.register.email })
      } catch (err) {
        error = err
      }
  ...
  describe('POST /users/refresh-token', () => {
    ...
    before(async () => {
      tokens = await loginAndGetTokens({ email: 'test@user.com', password: '123456aA@' })
    })
  ...
  describe('POST /users/change-email', () => {
    ...
    before(async () => {
      const tokens = await loginAndGetTokens({ email: context.changeEmail.email, password: 'Change123!@#' })
    ...
    it('initiates email change successfully', async () => {
      ...
      const verificationToken = await waitForVerificationToken({ email: newEmail, type: 'user_verification' })
      context.changeEmail.verificationToken = verificationToken.token
      context.changeEmail.authHeaders = changeEmailHeaders
    })
  ...
  describe('POST /users/verify-change-email', () => {
    it('verifies changed email successfully', async () => {
      const response = await api.post(
        ...
        context.changeEmail.authHeaders
      )

      expect(response.status).to.equal(200)
      const meResponse = await api.get('/users/me', context.changeEmail.authHeaders)
      expect(meResponse.data.data.email).to.equal(context.changeEmail.newEmail)
    ...
    it('returns error for invalid verification token', async () => {
      ...
      try {
        await api.post('/users/verify-change-email', { token: 'invalid-token' }, context.changeEmail.authHeaders)
      } catch (err) {
        error = err
      }
  ...
  describe('POST /users/cancel-change-email', () => {
    ...
    before(async () => {
      const tokens = await loginAndGetTokens({ email: context.changeEmail.newEmail, password: 'Change123!@#' })
    ...
    it('cancels email change successfully', async () => {
      ...
      expect(response.status).to.equal(200)
      const meResponse = await api.get('/users/me', cancelHeaders)
      expect(meResponse.data.data.new_email).to.equal(null)
    })
  ...
  describe('POST /users/set-user-email', () => {
    ...
    it('returns error when new email already exists', async () => {
      ...
      try {
        await api.post(
          '/users/set-user-email',
          { new_email: context.adminManaged.email, user_id: context.register.userId },
          { headers: { Authorization: authToken } }
        )
      } catch (err) {
        error = err
  ...
  describe('POST /users/change-password', () => {
    it('changes password successfully', async () => {
      const tokens = await loginAndGetTokens({ email: context.password.email, password: 'Password123!@#' })
      const newPassword = 'Password321!@#'
      const response = await api.post(
        '/users/change-password',
        { new_password: newPassword, old_password: 'Password123!@#' },
        { headers: { Authorization: tokens.access_token } }
      )
    ...
    it('returns error when new password matches old password', async () => {
      ...
      const headers = { headers: { Authorization: tokens.access_token } }
  ...
  describe('POST /users/set-user-password', () => {
    ...
    it('returns error when password is missing', async () => {
      ...
      try {
        await api.post(
          '/users/set-user-password',
          { user_id: context.adminManaged.userId },
          { headers: { Authorization: authToken } }
        )
      } catch (err) {
        error = err
  ...
  describe('POST /users/forgot-password', () => {
    it('triggers forgot password successfully', async () => {
      ...
      expect(response.status).to.equal(200)
      context.forgot.tokenDoc = await waitForVerificationToken({ email: context.forgot.email, type: 'forgot_password' })
    })
    ...
    it('returns error when user is not found', async () => {
      ...
      expect(error?.response?.status).to.equal(500)
      expect(error?.response?.data?.message).to.equal('USER_NOT_FOUND')
    })
  })
  ...
  describe('POST /users/verify-forgot-password', () => {
    ...
    it('returns error when token is invalid after use', async () => {
      let error
